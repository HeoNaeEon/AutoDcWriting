...
class API:
    ...
    async def remove_document(self, board_id, document_id, password="", is_minor=False):
        ...
        if res.find("true") < 0:
            raise Exception("Error while removing: " + unquote(str(res)))
        return True
    async def write_document(self, board_id, title="", contents="", name="", password="",pr="", is_minor=False):
        return await self.__write_or_modify_document(board_id, title, contents, name, password,pr, is_minor=is_minor)
    async def __write_or_modify_document(self, board_id, title="", contents="", name="", password="",pr="", intermediate=None, intermediate_referer=None, document_id=None, is_minor=False):
        if not intermediate:
        ...
        header = POST_HEADERS.copy()
        url = "http://mupload.dcinside.com/write_new.php"
